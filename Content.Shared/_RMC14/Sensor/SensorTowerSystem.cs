...
public sealed class SensorTowerSystem : EntitySystem
{
    ...
    private void OnSensorTowerInteractHand(Entity<SensorTowerComponent> ent, ref InteractHandEvent args)
    {
        ...
        if (!_skills.HasSkill(user, ent.Comp.Skill, ent.Comp.SkillLevel))
        {
            _popup.PopupClient(Loc.GetString("ssmc-bioscan-skill-issue"), ent, user, PopupType.SmallCaution);
            return;
        }
        ...
        {
            SensorTowerState.Weld => Loc.GetString("ssmc-bioscan-repair-welder-wirecut-wrench"),
            SensorTowerState.Wire => Loc.GetString("ssmc-bioscan-repair-wirecut-wrench"),
            SensorTowerState.Wrench => Loc.GetString("ssmc-bioscan-repair-wrench"),
            SensorTowerState.Off => Loc.GetString("ssmc-bioscan-lights-up", ("name", Name(ent))),
            SensorTowerState.On => Loc.GetString("ssmc-bioscan-goes-dark", ("name", Name(ent))),
            _ => throw new ArgumentOutOfRangeException(),
        };
    ...
    private void OnSensorTowerExamined(Entity<SensorTowerComponent> ent, ref ExaminedEvent args)
    {
        ...
        using (args.PushGroup(nameof(SensorTowerComponent)))
        {
            ...
            {
                SensorTowerState.Weld => Loc.GetString("ssmc-bioscan-damaged-repair-welder-wirecut-wrench"),
                SensorTowerState.Wire => Loc.GetString("ssmc-bioscan-damaged-repair-wirecut-wrench"),
                SensorTowerState.Wrench => Loc.GetString("ssmc-bioscan-damaged-repair-wrench"),
                SensorTowerState.Off => Loc.GetString("ssmc-bioscan-status-on"),
                SensorTowerState.On => Loc.GetString("ssmc-bioscan-status-off"),
                _ => throw new ArgumentOutOfRangeException(),
            };
            args.PushText(text);
            ...
            if (ent.Comp.State < SensorTowerState.Off)
            {
                ...
                {
                    SensorTowerState.Wrench => Loc.GetString("ssmc-apc-use-repair-wrench"),
                    SensorTowerState.Wire => Loc.GetString("ssmc-apc-use-repair-wirecutters"),
                    SensorTowerState.Weld => Loc.GetString("ssmc-apc-use-repair-welder"),
                    _ => throw new ArgumentOutOfRangeException(),
                };

                args.PushMarkup(Loc.GetString("ssmc-apc-use-repair-tool", ("tool", tool)));
            }
        }
    }
    ...
    private void Destroy(Entity<SensorTowerComponent> tower, EntityUid user)
    {
        ...
        {
            _popup.PopupClient(Loc.GetString("ssmc-bioscan-xeno-issue"), user, user, PopupType.SmallCaution);
            return;
        }
        ...
        {
            _popup.PopupClient(Loc.GetString("ssmc-bioscan-xeno-breach", ("tower", Name(tower))), tower, user, PopupType.Medium);
        }
    }
    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var uid, out var tower))
        {
            ...
            {
                _popup.PopupEntity(Loc.GetString("ssmc-bioscan-breach-beep", ("name", Name(uid))), uid, uid, PopupType.LargeCaution);
                tower.State = SensorTowerState.Wrench;
                Dirty(uid, tower);
            }
            else
            {
                _popup.PopupEntity(Loc.GetString("ssmc-bioscan-breach-beep-wirecut", ("name", Name(uid))), uid, uid, PopupType.LargeCaution);
