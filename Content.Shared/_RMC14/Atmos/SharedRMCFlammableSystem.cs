...
public abstract class SharedRMCFlammableSystem : EntitySystem
{
    ...
    private void OnCraftsIntoMolotovExamined(Entity<CraftsIntoMolotovComponent> ent, ref ExaminedEvent args)
    {
        ...
        {
            args.PushMarkup(Loc.GetString("ssmc-flame-push-you-can-do-molotov"));
        }
    }
    ...
    private void OnCanBeFirePattedInteractHand(Entity<CanBeFirePattedComponent> ent, ref InteractHandEvent args)
    {
        ...
        _popup.PopupClient(Loc.GetString("ssmc-flame-you-try-put-out-fire",("name", Name(ent))), ent, user, PopupType.SmallCaution);
        _popup.PopupEntity(Loc.GetString("ssmc-flame-smbd-try-put-you-out-fire", ("user", Name(user))), ent, ent, PopupType.SmallCaution);

        var others = Filter.PvsExcept(ent).RemoveWhereAttachedEntity(e => e == user || e == ent.Owner);
        _popup.PopupEntity(Loc.GetString("ssmc-flame-smbd-try-put-smb-out-fire",("user", Name(user)), ("target", Name(ent))), ent, others, true);
    ...
    private bool CanCraftMolotovPopup(Entity<CraftsIntoMolotovComponent> ent, EntityUid user, bool popup, out FixedPoint2 intensity)
    {
        ...
        {
            if (popup)
                _popup.PopupClient(Loc.GetString("ssmc-flame-smth-is-empty", ("name", Name(ent))), ent, user, PopupType.SmallCaution);
        ...
        if (intensity < ent.Comp.MinIntensity)
        {
            if (popup)
            {
                var msg = Loc.GetString("ssmc-flame-no-big-tank-napalm", ("name", Name(ent)));
