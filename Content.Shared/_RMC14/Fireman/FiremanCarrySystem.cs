...
public sealed class FiremanCarrySystem : EntitySystem
{
    ...
    private void OnCarriableDragDropDragged(Entity<FiremanCarriableComponent> ent, ref DragDropDraggedEvent args)
    {
        ...
        {
            _popup.PopupClient(Loc.GetString("ssmc-interactive-fireman-skill-issue"), ent, user, PopupType.MediumCaution);
            return;
        }

        if (!carrier.AggressiveGrab)
        {
            _popup.PopupClient(Loc.GetString("ssmc-interactive-fireman-agressive-grab"), ent, user, PopupType.MediumCaution);
            return;
        }
        ...
        _popup.PopupClient(Loc.GetString("ssmc-interactive-fireman-process", ("target", target)), ent, user, PopupType.Medium);
    }
