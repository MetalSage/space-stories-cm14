...
public sealed class RMCBattleExecuteSystem : EntitySystem
{
        ...
        EntityUid handHeldItem)
    {
        ...
        {
            var cancelledMessage = Loc.GetString("ssmc-execute-no-defib-cancel", ("target", Name(target)));
    ...
    private void ExecuteDoAfter(Entity<MarineComponent> ent, ref RMCBattleExecuteEvent args)
    {
        ...
        if (args.Cancelled)
        {
            ...
            var cancelledMessage = Loc.GetString("ssmc-execute-cancel", ("target", Name(target)));
            _popup.PopupClient(cancelledMessage, user, PopupType.MediumCaution);
            return;
        }
        ...
        var popupMessage = Loc.GetString("ssmc-execute-was-executed-by", ("target", Name(target)), ("name", Name(user)));
        _popup.PopupPredicted(popupMessage, target, user, PopupType.LargeCaution);

        var chatMsg = $"\n {Loc.GetString("ssmc-execute-was-executed-by", ("target", Name(target)), ("name", Name(user)))} \n";
