...
public abstract class SharedXenoWeedsSystem : EntitySystem
{
        ...
        bool limitDistance)
    {
        if (_rmcMap.HasAnchoredEntityEnumerator<XenoWeedsComponent>(coordinates, out var oldWeeds))
        {
            ...
            {
                _popup.PopupClient(Loc.GetString("ssmc-xeno-weeds-here-already"), oldWeeds, xeno, PopupType.SmallCaution);
                return false;
            }

            if (oldWeeds.Comp.BlockOtherWeeds)
            {
                _popup.PopupClient(Loc.GetString("ssmc-xeno-weeds-too-strong-for-node"),
                    oldWeeds,
                    xeno,
        ...
        {
            _popup.PopupClient(Loc.GetString("ssmc-xeno-weeds-only-near-our-hive"),
                xeno,
                xeno,
                PopupType.SmallCaution);
            return false;
