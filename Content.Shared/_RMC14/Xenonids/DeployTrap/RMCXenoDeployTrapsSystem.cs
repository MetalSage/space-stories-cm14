...
public sealed class RMCXenoDeployTrapsSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<RMCXenoDeployedTrapComponent, StartCollideEvent>(OnTrapStartCollide);
    }

    public bool IsTrapped(EntityUid entityUid)
    {
        return HasComp<RMCXenoDeployedTrappedComponent>(entityUid);
    }
    ...
    private void OnDeployTrap(Entity<RMCXenoDeployTrapsComponent> ent, ref RMCXenoDeployTrapsActionEvent args)
    {
        ...
        var empowered = _acidInsight.TryUseEmpower(ent.Owner);
        ...
        {
            var trapUid = Spawn(ent.Comp.PrototypeId, new EntityCoordinates(args.Target.EntityId, start + axis * i));
            var trapComponent = EnsureComp<RMCXenoDeployedTrapComponent>(trapUid);
            trapComponent.Empowered = empowered;
            DirtyField(trapUid, trapComponent, nameof(RMCXenoDeployedTrapComponent.Empowered));
        }
    }

    private void OnProjectileHit(Entity<RMCXenoDeployTrapsComponent> ent, ref XenoProjectileHitUserEvent args)
    {
        if (!TryComp<RMCXenoDeployedTrappedComponent>(args.Hit, out var deployedTrappedComponent))
            return;
    ...
    private void OnTrapStartCollide(Entity<RMCXenoDeployedTrapComponent> ent, ref StartCollideEvent args)
    {
        if (!_xeno.CanAbilityAttackTarget(ent, args.OtherEntity))
            return;

        var targetUid = args.OtherEntity;
        _slow.TryRoot(targetUid, ent.Comp.CurrentRootDuration);

        EnsureComp<RMCXenoDeployedTrappedComponent>(targetUid);

        if (_net.IsClient)
            return;

        Timer.Spawn(ent.Comp.CurrentRootDuration, () =>
        {
            RemCompDeferred<RMCXenoDeployedTrappedComponent>(targetUid);
        });
