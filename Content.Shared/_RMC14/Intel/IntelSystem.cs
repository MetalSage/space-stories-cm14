...
public sealed class IntelSystem : EntitySystem
{
    ...
    private void OnReadUseInHand(Entity<IntelReadObjectiveComponent> ent, ref UseInHandEvent args)
    {
        ...
        if (_doAfter.TryStartDoAfter(doAfter))
            _popup.PopupClient(Loc.GetString("ssmc-interactive-start-reading-intel", ("name", Name(ent))), ent, user);
    }
    ...
    private void OnReadDoAfter(Entity<IntelReadObjectiveComponent> ent, ref IntelReadDoAfterEvent args)
    {
        ...
        if (args.Cancelled)
        {
            _popup.PopupClient(Loc.GetString("ssmc-interactive-lose-thought"), ent, user);
            return;
        }

        if (ent.Comp.State == IntelObjectiveState.Inactive)
        {
            _popup.PopupClient(Loc.GetString("ssmc-interactive-nothing-useful"), ent, user);
            return;
        }

        // TODO RMC14 clues

        _popup.PopupClient(Loc.GetString("ssmc-interactive-finish-reading-intel", ("name", Name(ent))), ent, user);
    ...
    {
        var msg = Loc.GetString("ssmc-interactive-start-typing-comp");
        if (!TryComp(args.User, out IntelKnowledgeComponent? knowledge) ||
            !knowledge.Read.TryFirstOrNull(out var read))
        {
            msg += Loc.GetString("ssmc-interactive-nothing-add-to-comp");
    ...
    private void OnConsoleSubmitDoAfter(Entity<IntelConsoleComponent> ent, ref IntelSubmitDoAfterEvent args)
    {
        ...
        {
            _popup.PopupEntity(
                Loc.GetString("ssmc-interactive-lose-thought-comp"),
                ent,
                args.User,
        ...
        {
            if (args.Amount == 0)
                _popup.PopupEntity(Loc.GetString("ssmc-interactive-nothing-add-to-comp"), ent, args.User, PopupType.Medium);
            else
                _popup.PopupEntity(Loc.GetString("ssmc-interactive-added-to-comp", ("count", args.Amount)), ent, args.User, PopupType.Medium);
        }
