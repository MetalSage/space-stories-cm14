...
public sealed class CMArmorSystem : EntitySystem
{
    ...
    public void UpdateArmorValue(Entity<CMArmorComponent?> armored)
    {
        ...
        var armorMessage = ev.FrontalArmor == 0 &&
            ...
            : Loc.GetString("ssmc-xeno-armor-overall", ("armor", FixedPoint2.New(ev.XenoArmor * ev.ArmorModifier)), ("armor2", armored.Comp.XenoArmor));

        if (armored.Comp.FrontalArmor != 0 || ev.FrontalArmor != 0)
            armorMessage = $" {armorMessage} /n {Loc.GetString("ssmc-xeno-armor-frontal", ("armor", FixedPoint2.New((ev.XenoArmor + ev.FrontalArmor) * ev.ArmorModifier)), ("armor2", armored.Comp.XenoArmor + armored.Comp.FrontalArmor))}";

        if (armored.Comp.SideArmor != 0 || ev.SideArmor != 0)
            armorMessage = $" {armorMessage} /n {Loc.GetString("ssmc-xeno-armor-side", ("armor", FixedPoint2.New((ev.XenoArmor + ev.SideArmor) * ev.ArmorModifier)), ("armor2", armored.Comp.XenoArmor + armored.Comp.SideArmor))}";
