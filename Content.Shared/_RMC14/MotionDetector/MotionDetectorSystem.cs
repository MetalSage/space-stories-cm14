...
public sealed class MotionDetectorSystem : EntitySystem
{
    ...
    private void OnMotionDetectorGetVerbs(Entity<MotionDetectorComponent> ent, ref GetVerbsEvent<AlternativeVerb> args)
    {
        ...
        {
            Text = ent.Comp.Short ? Loc.GetString("ssmc-detector-change-to-long-mode") : Loc.GetString("ssmc-detector-change-to-short-mode"),
            Act = () =>
            {
                ent.Comp.Short = !ent.Comp.Short;
                Dirty(ent);
                _audio.PlayPredicted(ent.Comp.ToggleSound, ent, user);
                _popup.PopupClient(Loc.GetString("ssmc-detector-changed-to-mode", ("name", Name(ent)), ("mode", (ent.Comp.Short ? Loc.GetString("ssmc-detector-short-mode") : Loc.GetString("ssmc-detector-long-mode")))), ent, user);
            },
        });
    }
    ...
    private void OnMotionDetectorExamined(Entity<MotionDetectorComponent> ent, ref ExaminedEvent args)
    {
        ...
        {
            var mode = ent.Comp.Short ?  Loc.GetString("ssmc-detector-short-mode") :  Loc.GetString("ssmc-detector-long-mode");
            args.PushMarkup(Loc.GetString("ssmc-detector-mode", ("mode", mode)));
        }
    }
