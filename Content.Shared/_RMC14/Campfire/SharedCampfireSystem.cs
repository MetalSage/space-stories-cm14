...
public abstract class SharedCampfireSystem : EntitySystem
{
        ...
        while (query.MoveNext(out var uid, out var comp))
        {
            ...
            if (elapsed >= comp.BurnDuration)
            {
                ...
                else
                {
                    ...
                    if (_net.IsServer)
                        _popup.PopupEntity(Loc.GetString("ssmc-campfire-fire-goes-out"), uid);
                }
            }
        }
    }
    ...
    private void OnActivateInWorld(Entity<CampfireComponent> ent, ref ActivateInWorldEvent args)
    {
        ...
        {
            if (_net.IsServer)
                _popup.PopupEntity(Loc.GetString("ssmc-campfire-extinguishing-fire"), ent, args.User);
        }
    }
    ...
    private void OnExtinguishDoAfter(Entity<CampfireComponent> ent, ref CampfireExtinguishDoAfterEvent args)
    {
        ...
        if (_net.IsServer)
            _popup.PopupEntity(Loc.GetString("ssmc-campfire-extinguishing-fire-done"), ent, args.User);
    }
    ...
    private void OnInteractUsing(Entity<CampfireComponent> ent, ref InteractUsingEvent args)
    {
        ...
        if (TryComp<CampfireFuelComponent>(args.Used, out var fuelComp))
        {
            ...
            {
                if (_net.IsServer)
                    _popup.PopupEntity(Loc.GetString("ssmc-campfire-full-fuel"), ent, args.User);
                return;
            }
            ...
            if (_net.IsServer)
                _popup.PopupEntity(Loc.GetString("ssmc-campfire-adding-fuel"), ent, args.User);
        ...
        {
            if (_net.IsServer)
                _popup.PopupEntity(Loc.GetString("ssmc-campfire-need-fuel"), ent, args.User);
            return;
        }
    ...
    public void SetLit(Entity<CampfireComponent> ent, bool lit, EntityUid? user = null)
    {
        ...
        if (lit)
        {
            ...
            if (user != null)
                _popup.PopupEntity(Loc.GetString("ssmc-campfire-light-fire"), ent, user.Value);
        }
