...
public sealed class VisorSystem : EntitySystem
{
    ...
    private void OnCycleableVisorInteractUsing(Entity<CycleableVisorComponent> ent, ref InteractUsingEvent args)
    {
        ...
        if (anyRemoved)
            _popup.PopupClient(Loc.GetString("ssmc-visor-remove-inserted"), args.Target, args.User);
        else
            _popup.PopupClient(Loc.GetString("ssmc-visor-remove-no-one"), args.Target, args.User);
        ...
        EntityUid? user)
    {
        ...
        foreach (var id in cycleable.Comp.Containers)
        {
            ...
            {
                msg = Loc.GetString("ssmc-visor=connect-to", ("visor", Name(visor)), ("target", Name(cycleable)));
                _popup.PopupClient(msg, cycleable, user);
                return true;
            }
        }

        msg = Loc.GetString("ssmc-visor-no-more-slots", ("helmet", Name(cycleable)));
