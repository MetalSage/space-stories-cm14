...
public abstract class SharedRMCChemistrySystem : EntitySystem
{
    ...
    private void OnDetailedSolutionExamined(Entity<DetailedExaminableSolutionComponent> ent, ref ExaminedEvent args)
    {
        ...
        {
            args.PushText(Loc.GetString("ssmc-chem-disp-contents"));
            if (!_solution.TryGetSolution(ent.Owner, ent.Comp.Solution, out _, out var solution) ||
                solution.Volume <= FixedPoint2.Zero)
            {
                args.PushText(Loc.GetString("ssmc-chem-nothing"));
            }
            else
            {
                foreach (var reagent in solution.Contents)
                {
                    ...
                    args.PushText(Loc.GetString("ssmc-chem-disp-reagent-quantity", ("reagentquantity", reagent.Quantity.Float()), ("reagentname", name)));
                }

                args.PushText(Loc.GetString("ssmc-chem-total-volume", ("units", solution.Volume), ("max", solution.MaxVolume)));
            }
            ...
            if (TryComp<RMCToggleableSolutionTransferComponent>(ent.Owner, out var transferComp))
            {
                ...
                {
                    SolutionTransferDirection.Input => Loc.GetString("ssmc-chem-transfer-mode-drawing"),
                    SolutionTransferDirection.Output => Loc.GetString("ssmc-chem-transfer-mode-dispersing"),
                    _ => string.Empty,
                };
    ...
    private void OnToggleableSolutionTransferVerbs(Entity<RMCToggleableSolutionTransferComponent> ent, ref GetVerbsEvent<AlternativeVerb> args)
    {
        ...
        {
            Text = dispensing ? Loc.GetString("ssmc-chem-transfer-mode-enable-drawing"): Loc.GetString("ssmc-chem-transfer-mode-enable-dispersing"),
            Act = () =>
            {
                ...
                if (dispensing)
                {
                    ...
                    ent.Comp.Direction = SolutionTransferDirection.Input;
                    Dirty(ent, refillable);
                    _popup.PopupClient(Loc.GetString("ssmc-chem-transfer-mode-drawning"), ent, user, PopupType.Medium);
                }
                else
                {
                    ...
                    Dirty(ent, drainable);
                    _popup.PopupClient(Loc.GetString("ssmc-chem-transfer-mode-dispersing"), ent, user, PopupType.Medium);
                }
            },
        });
    }
