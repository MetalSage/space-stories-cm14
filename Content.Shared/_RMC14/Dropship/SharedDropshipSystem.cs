...
public abstract class SharedDropshipSystem : EntitySystem
{
    ...
    private void OnDropshipTerminalActivateInWorld(Entity<DropshipTerminalComponent> ent, ref ActivateInWorldEvent args)
    {
        ...
        {
            _popup.PopupEntity(Loc.GetString("ssmc-dropship-hijack-glupy", ("name", Name(ent.Owner))), user, user);
            return;
        }
        ...
        {
            _popup.PopupEntity(Loc.GetString("ssmc-dropship-no-dropship-destinations"), user, user, PopupType.MediumCaution);
            return;
        }

        if (closestDestination.Value.Comp1.Ship != null)
        {
            _popup.PopupEntity(Loc.GetString("ssmc-dropship-hijack-dropship-coming-here"), user, user, PopupType.MediumCaution);
            return;
        }
        ...
        {
            _popup.PopupEntity(Loc.GetString("ssmc-dropship-hijack-not-primary-lz"), user, user, PopupType.MediumCaution);
            return;
        }
        ...
        while (dropships.MoveNext(out var uid, out var dropship, out var xform))
        {
            ...
            while (computerQuery.MoveNext(out var computerId, out var computer))
            {
                ...
                {
                    _popup.PopupEntity(Loc.GetString("ssmc-dropship-hijack-shuttle-called-done"), user, user, PopupType.LargeCaution);
                    return;
                }
            }
        }

        _popup.PopupEntity(Loc.GetString("ssmc-dropship-hijack-no-available"), user, user, PopupType.LargeCaution);
    }
    ...
    private void OnTerminalSummon(Entity<DropshipTerminalComponent> terminal, ref DropshipTerminalSummonDropshipMsg args)
    {
        ...
        if (closestDestination == null)
        {
            _popup.PopupEntity(Loc.GetString("ssmc-dropship-hijack-no-dropship-destinations"), terminal, args.Actor, PopupType.MediumCaution);
            return;
        }
        ...
        if (closestDestination.Value.Comp1.Ship is { } ship)
        {
            ...
            {
                _popup.PopupEntity(Loc.GetString("ssmc-dropship-hijack-dropship-coming-here"), terminal, args.Actor, PopupType.MediumCaution);
            }
            else
            {
                _popup.PopupEntity(Loc.GetString("ssmc-dropship-hijack-shuttle-already-here"), terminal, args.Actor, PopupType.MediumCaution);
            }
            return;
        }
        ...
        {
            _popup.PopupEntity(Loc.GetString("ssmc-dropship-remote-control-no-enabled"), terminal, args.Actor, PopupType.MediumCaution);
            return;
        }
        ...
        {
            _popup.PopupEntity(Loc.GetString("ssmc-dropship-remote-control-busy"), terminal, args.Actor, PopupType.MediumCaution);
            return;
        }

        _ui.CloseUi(terminal.Owner, DropshipTerminalUiKey.Key, args.Actor);
        _popup.PopupEntity(Loc.GetString("ssmc-dropship-hijack-dropship-coming-here"), terminal, args.Actor, PopupType.Medium);
    }
