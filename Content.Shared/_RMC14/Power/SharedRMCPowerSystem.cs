...
public abstract class SharedRMCPowerSystem : EntitySystem
{
    ...
    private void OnApcInteractUsing(Entity<RMCApcComponent> ent, ref InteractUsingEvent args)
    {
        ...
        {
            _popup.PopupClient(Loc.GetString("ssmc-apc-skill-issue", ("name", Name(ent))), ent, user, SmallCaution);
            return;
        }
        ...
        if (_tool.HasQuality(used, ent.Comp.CrowbarTool))
        {
            switch (ent.Comp.State)
            {
                ...
                case RMCApcState.WiresExposed:
                    ...
                    {
                        _popup.PopupClient(Loc.GetString("ssmc-apc-cover-lock-try"), user, user, MediumCaution);
                        return;
                    }
    ...
    private void OnApcActivatableUIOpenAttempt(Entity<RMCApcComponent> ent, ref ActivatableUIOpenAttemptEvent args)
    {
        ...
        {
            args.Cancel();
            _popup.PopupClient(Loc.GetString("ssmc-apc-skill-issue", ("name", Name(ent))), ent, args.User, SmallCaution);
            return;
        }
    ...
    private void OnApcExamined(Entity<RMCApcComponent> ent, ref ExaminedEvent args)
    {
        ...
        using (args.PushGroup(nameof(RMCApcComponent)))
        {
            var markup = ent.Comp.State switch
            {
                ...
                RMCApcState.WiresExposed => Loc.GetString("ssmc-apc-screw-crowbar"),
                RMCApcState.CoverOpenBattery => Loc.GetString("ssmc-apc-hand-crowbar"),
                RMCApcState.CoverOpenNoBattery => Loc.GetString("ssmc-apc-battery-slot"),
                _ => null,
            };
    ...
    private void OnFusionReactorExamined(Entity<RMCFusionReactorComponent> ent, ref ExaminedEvent args)
    {
        ...
        using (args.PushGroup(nameof(RMCFusionReactorComponent)))
        {
            if (ent.Comp.State != RMCFusionReactorState.Working)
            {
                ...
                {
                    RMCFusionReactorState.Wrench => Loc.GetString("ssmc-apc-use-repair-wrench"),
                    RMCFusionReactorState.Wire => Loc.GetString("ssmc-apc-use-repair-wirecutters"),
                    RMCFusionReactorState.Weld => Loc.GetString("ssmc-apc-use-repair-welder"),
                    _ => throw new ArgumentOutOfRangeException(),
                };
            ...
            {
                // TODO: localize
                args.PushMarkup(Loc.GetString("ssmc-apc-need-battery"));
            }
        }
    }
