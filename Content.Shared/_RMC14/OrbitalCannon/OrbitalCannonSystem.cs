...
public sealed class OrbitalCannonSystem : EntitySystem
{
    ...
    private void OnWarheadPowerLoaderInteract(Entity<OrbitalCannonWarheadComponent> ent, ref PowerLoaderInteractEvent args)
    {
            ...
            cannon.Status != OrbitalCannonStatus.Unloaded)
        {
            ...
            {
                _popup.PopupClient(Loc.GetString("ssmc-orbital-cannon-tray-already-loadded"), args.Target, buckled, PopupType.MediumCaution);
            }
        ...
        if (container.ContainedEntity != null)
        {
            foreach (var buckled in args.Buckled)
            {
                _popup.PopupClient(Loc.GetString("ssmc-orbital-cannon-warhead-already-loaded"), args.Target, buckled, PopupType.MediumCaution);
            }
        ...
        if (!_container.Insert(args.Used, container))
        {
            foreach (var buckled in args.Buckled)
            {
                _popup.PopupClient(Loc.GetString("ssmc-orbital-cannon-no-insert-into", ("item", Name(args.Used)), ("target", Name(args.Target))), args.Target, buckled, PopupType.MediumCaution);
            }
        }

        _popup.PopupClient(Loc.GetString("ssmc-orbital-cannon-insert-into", ("item", Name(args.Used)), ("target", Name(args.Target))), args.Target, args.Target, PopupType.Medium);
    ...
    private void OnFuelPowerLoaderInteract(Entity<OrbitalCannonFuelComponent> ent, ref PowerLoaderInteractEvent args)
    {
            ...
            cannon.Status != OrbitalCannonStatus.Unloaded)
        {
            foreach (var buckled in args.Buckled)
            {
                _popup.PopupClient(Loc.GetString("ssmc-orbital-cannon-tray-already-loadded"), buckled, PopupType.MediumCaution);
            }
            ...
            warheadContainer.ContainedEntities.Count == 0)
        {
            foreach (var buckled in args.Buckled)
            {
                _popup.PopupClient(Loc.GetString("ssmc-orbital-cannon-place-warhead-first", ("target", Name(args.Target))), args.Target, buckled, PopupType.MediumCaution);
            }
        ...
        if (fuelContainer.ContainedEntities.Count >= tray.MaxFuel)
        {
            foreach (var buckled in args.Buckled)
            {
                _popup.PopupClient(Loc.GetString("ssmc-orbital-cannon-no-more-fuel", ("name", Name(args.Target))), args.Target, buckled, PopupType.MediumCaution);
            }
    ...
    public bool Fire(Entity<OrbitalCannonComponent> cannon, Vector2i fireCoordinates, EntityUid user, EntityUid squad)
    {
        ...
        if (cannon.Comp.LinkedTray is not { } trayId || !TryComp(trayId, out OrbitalCannonTrayComponent? tray))
        {
            _popup.PopupCursor(Loc.GetString("ssmc-orbital-cannon-no-linked-tray"), user, PopupType.LargeCaution);
            return false;
        }
        ...
        if (!_container.TryGetContainer(trayId, tray.WarheadContainer, out var warheadContainer) ||
            warheadContainer.ContainedEntities.Count == 0)
        {
            _popup.PopupCursor(Loc.GetString("ssmc-orbital-cannon-no-ammo"), user, PopupType.LargeCaution);
            return false;
        }

        if (!_rmcPlanet.TryPlanetToCoordinates(fireCoordinates, out var planetCoordinates))
        {
            _popup.PopupCursor(Loc.GetString("ssmc-orbital-cannon-coord-out-of-bounds"), user, PopupType.LargeCaution);
            return false;
        }
        ...
        if (!_area.CanOrbitalBombard(_transform.ToCoordinates(planetCoordinates), out var roofed))
        {
            if (roofed)
            {
                _popup.PopupCursor(Loc.GetString("ssmc-orbital-cannon-bio-protect"), user, PopupType.LargeCaution);
            ...
            _popup.PopupCursor(Loc.GetString("ssmc-orbital-cannon-deep-underground"), user, PopupType.LargeCaution);
            return false;
        }

        _popup.PopupCursor(Loc.GetString("ssmc-orbital-cannon-accepted"), PopupType.Large);
        ...
        _popup.PopupCursor(Loc.GetString("ssmc-orbital-cannon-started"), user);
    ...
    public override void Update(float frameTime)
    {
        ...
        while (firingQuery.MoveNext(out var uid, out var firing, out var cannon))
        {
            ...
            if (!firing.Alerted && time > firing.StartedAt + firing.AlertDelay)
            {
                ...
                var msg = Loc.GetString("ssmc-orbital-cannon-start-detected");
                ...
                {
                    msg = Loc.GetString("ssmc-orbital-cannon-type-and-area", ("name", firing.WarheadName), ("area", areaProto.Name));
                    _rmcChat.ChatMessageToMany(msg, msg, groundFilter, ChatChannel.Radio);
                }
            }
            ...
            if (!firing.BegunFire && time > firing.StartedAt + firing.BeginFireDelay)
            {
                ...
                var msg = Loc.GetString("ssmc-orbital-cannon-almayer-announce");
                _rmcChat.ChatMessageToMany(msg, msg, sameMap, ChatChannel.Radio);

                _marineAnnounce.AnnounceSquad(Loc.GetString("ssmc-orbital-cannon-squad-announce "), firing.Squad);
            }
