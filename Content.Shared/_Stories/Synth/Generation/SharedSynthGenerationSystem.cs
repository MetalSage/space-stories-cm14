...
public sealed class SharedSynthGenerationSystem : EntitySystem
{
    ...
    public void SynthStartup(Entity<SynthComponent> ent)
    {
        ...
        if (comp.Generation is { } generation)
        {
            ...
            {
                RemComp<MovementSpeedModifierComponent>(ent.Owner);
    ...
    {
        if (ent.Comp.Generation is { } setGeneration && ent.Comp.SelectGenerationActionEntity != null)
        {
            if (_prototype.TryIndex(setGeneration, out var setProto))
            {
                RemComp<MovementSpeedModifierComponent>(ent.Owner);
                EntityManager.AddComponents(ent.Owner, setProto);
                _movementSpeed.RefreshMovementSpeedModifiers(ent.Owner);
            }

            ApplyGenerationModifier(ent);
            _actions.RemoveAction(ent.Owner, ent.Comp.SelectGenerationActionEntity);
            ent.Comp.SelectGenerationActionEntity = null;
            Dirty(ent);
        }
        ...
        if (ent.Comp.Generation is not { } current)
            return;

        if (_prototype.TryIndex<EntityPrototype>(current, out var currentProto) &&
