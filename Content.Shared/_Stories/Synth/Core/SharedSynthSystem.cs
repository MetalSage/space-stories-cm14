...
public abstract class SharedSynthSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<SynthComponent, STSynthRepairEvent>(OnSynthRepairDoAfter);
    ...
    private void OnSynthInteractUsing(Entity<SynthComponent> synth, ref InteractUsingEvent args)
    {
        ...
        if (HasComp<BlowtorchComponent>(used) && _tool.HasQuality(used, synth.Comp.RepairQuality))
        {
            ...
            if (_doAfter.TryStartDoAfter(doAfter))
            {
                ...
                var selfMsg = Loc.GetString(selfRepair ? "st-synth-repair-brute-start-self" : "st-synth-repair-brute-start-target-self",
                    ("user", user),
                    ("target", synth),
                    ("tool", used),
                    ("limb", "chest"));
                var othersMsg = Loc.GetString("st-synth-repair-brute-start-others", ("user", user), ("target", synth), ("tool", used), ("limb", "chest"));
        ...
        else if (HasComp<RMCCableCoilComponent>(used))
        {
            ...
            if (HasDamage(synth, synth.Comp.CableCoilDamageGroup))
            {
                if (_doAfter.TryStartDoAfter(doAfter))
                {
                    var selfMsg = Loc.GetString(selfRepair ? "st-synth-repair-burn-start-self" : "st-synth-repair-burn-start-target-self",
                        ("user", user),
                        ("target", synth),
                        ("tool", used),
                        ("limb", "chest"));
                    var othersMsg = Loc.GetString("st-synth-repair-burn-start-others", ("user", user), ("target", synth), ("tool", used), ("limb", "chest"));
    ...
    private void OnSynthRepairDoAfter(Entity<SynthComponent> synth, ref STSynthRepairEvent args)
    {
        ...
        if (HasComp<BlowtorchComponent>(used) && _repairable.UseFuel(used.Value, user, 5))
        {
            ...
            var selfMsg = Loc.GetString("st-synth-repair-brute-finish-self", ("user", user), ("target", synth), ("tool", used), ("limb", "chest"));
            var othersMsg = Loc.GetString("st-synth-repair-brute-finish", ("user", user), ("target", synth), ("tool", used), ("limb", "chest"));
            _popup.PopupPredicted(selfMsg, othersMsg, user, user);
        }
        else if (HasComp<RMCCableCoilComponent>(args.Used) && _stack.Use(args.Used.Value, 1))
        {
            ...
            var selfMsg = Loc.GetString("st-synth-repair-burn-finish-self", ("user", user), ("target", synth), ("tool", used), ("limb", "chest"));
            var othersMsg = Loc.GetString("st-synth-repair-burn-finish", ("user", user), ("target", synth), ("tool", used), ("limb", "chest"));
            _popup.PopupPredicted(selfMsg, othersMsg, user, user);
        }
    }
    ...
    public void DoSynthUnableToUsePopup(EntityUid synth, EntityUid tool)
    {
        ...
        var msg = Loc.GetString("st-species-synth-programming-prevents-use", ("user", synth), ("tool", tool));
        _popup.PopupClient(msg, synth, synth, PopupType.SmallCaution);
    }
}

[Serializable, NetSerializable]
public sealed partial class STSynthRepairEvent : SimpleDoAfterEvent;
