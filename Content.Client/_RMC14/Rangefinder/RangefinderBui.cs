...
public sealed class RangefinderBui : BoundUserInterface
{
    ...
    public void Refresh()
    {
        ...
        if (rangefinder.LastCoords is { } mapCoords)
        {
            ...
            _window.BottomContainer.AddChild(AddRow(Loc.GetString("ssmc-area-supply-drop"), _area.CanSupplyDrop(mapCoords)));
            _window.BottomContainer.AddChild(AddRow(Loc.GetString("ssmc-area-mortar"), _area.CanMortarFire(coords)));
            _window.BottomContainer.AddChild(AddRow(Loc.GetString("ssmc-area-cas"), _area.CanCAS(coords)));
            _window.BottomContainer.AddChild(AddRow(Loc.GetString("ssmc-area-ob"), _area.CanOrbitalBombard(coords, out _)));
        }
    }
