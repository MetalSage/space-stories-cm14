    ...
    public void Refresh()
    {
        ...
        switch (computer.Mode)
        {
            case EvacuationComputerMode.Disabled:
                _window.StatusLabel.Text = Loc.GetString("ssmc-evac-comp-status-label-delayed");
                _window.HatchLabel.Text = Loc.GetString("ssmc-evac-comp-hatch-label");
                break;
            case EvacuationComputerMode.Ready:
                _window.StatusLabel.Text = Loc.GetString("ssmc-evac-comp-status-label-stand-by");
                _window.HatchLabel.Text = Loc.GetString("ssmc-evac-comp-hatch-label-secured");
                break;
            case EvacuationComputerMode.Travelling:
                // TODO RMC14 launching
                _window.StatusLabel.Text = Loc.GetString("ssmc-evac-comp-status-label-travelling");
                _window.HatchLabel.Text = Loc.GetString("ssmc-evac-comp-hatch-label-secured");
                break;
            case EvacuationComputerMode.Crashed:
                _window.StatusLabel.Text = Loc.GetString("ssmc-evac-comp-status-label-crashed");
                _window.HatchLabel.Text = Loc.GetString("ssmc-evac-comp-hatch-label");
                break;
        }
    }
