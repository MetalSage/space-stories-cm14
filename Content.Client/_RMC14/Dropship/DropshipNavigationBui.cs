...
public sealed class DropshipNavigationBui : BoundUserInterface
{
    ...
    private void OpenWindow()
    {
        ...
        SetFlightHeader(Loc.GetString("ssmc-dropship-flight-controls"));
        SetDoorHeader(Loc.GetString("ssmc-dropship-door-controls"));
        SetRemoteControlHeader(Loc.GetString("ssmc-dropship-remote-control-header"));
    ...
    private void Set(DropshipNavigationDestinationsBuiState destinations)
    {
        ...
        {
            var flyByName = Loc.GetString("ssmc-dropship-flyby");
        ...
        foreach (var destination in destinations.Destinations)
        {
            ...
            if (destination.Primary)
                name += $" {Loc.GetString("ssmc-dropship-primary-lz")}";
    ...
    private void Set(DropshipNavigationTravellingBuiState travelling)
    {
        ...
        switch (travelling.State)
        {
            case FTLState.Starting:
                SetFlightHeader(Loc.GetString("ssmc-drophip-launch-in-progress"));
                _window.ProgressBarHeader.SetMarkup(Msg(Loc.GetString("ssmc-dropship-launch-to-lz-time", ("time", time), ("destination", destination))));
                SetLockDownDisabled(false);
                break;
            case FTLState.Travelling:
                SetFlightHeader(Loc.GetString("ssmc-dropship-in-flight-to", ("destination", destination)));
                _window.ProgressBarHeader.SetMarkup(Msg(Loc.GetString("ssmc-dropship-in-flight-time", ("time", time))));
                SetLockDownDisabled(true);
                SetCancelDisabled(false);
                break;
            case FTLState.Arriving:
                SetFlightHeader(Loc.GetString("ssmc-dropship-final-approach-to", ("destination", destination)));
                _window.ProgressBarHeader.SetMarkup(Msg(Loc.GetString("ssmc-dropship-final-approach-time", ("time", time))));
                SetLockDownDisabled(true);
                SetCancelDisabled(true);
                break;
            case FTLState.Cooldown:
                SetFlightHeader(Loc.GetString("ssmc-dropship-fuel-progress"));
                _window.ProgressBarHeader.SetMarkup(Msg(Loc.GetString("ssmc-dropship-fuel-progress-time", ("time", time))));
                SetLockDownDisabled(false);
                SetCancelDisabled(true);
                break;
            default:
