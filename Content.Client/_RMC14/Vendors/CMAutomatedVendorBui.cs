...
public sealed class CMAutomatedVendorBui : BoundUserInterface
{
            ...
            for (var sectionIndex = 0; sectionIndex < vendor.Sections.Count; sectionIndex++)
            {
                ...
                for (var entryIndex = 0; entryIndex < section.Entries.Count; entryIndex++)
                {
                    ...
                    if (_prototype.TryIndex(entry.Id, out var entity))
                    {
                        ...
                        {
                            name = Loc.GetString("ssmc-vendor-mandatory", ("name", name));
                        ...
                        else if (entry.Recommended)
                        {
                            ...
                            name = Loc.GetString("ssmc-vendor-recommended", ("name", name));
    ...
    public void Refresh()
    {
        ...
        _window.PointsLabel.Text = anyEntryWithPoints ? Loc.GetString("ssmc-vendor-points-remaining", ("points", userPoints)) : string.Empty;
        ...
        _window.ReagentsLabel.Text = Loc.GetString("ssmc-vendor-reagents-label", ("count", current.Int()));
    }
    ...
    private FormattedMessage GetSectionName(CMVendorUserComponent? user, CMVendorSection section)
    {
        ...
        if (section.TakeAll != null)
        {
            ...
            foreach (var entry in section.Entries)
            {
                ...
                {
                    name.AddText($" {Loc.GetString("ssmc-vendor-take-all")}");
                    break;
                }
            }
        }
        else if (section.TakeOne != null)
        {
            ...
            {
                name.AddText($" {Loc.GetString("ssmc-vendor-take-one")}");
            }
        }
        else if (section.Choices is { } choices)
        {
            if (user == null)
            {
                name.AddText($" {Loc.GetString("ssmc-vendor-choose-left", ("left", choices.Amount))}");
            }
            else
            {
                var left = choices.Amount - user.Choices.GetValueOrDefault(choices.Id);
                if (left > 0)
                    name.AddText($" {Loc.GetString("ssmc-vendor-choose-left", ("left", left))}");
            }
        }
