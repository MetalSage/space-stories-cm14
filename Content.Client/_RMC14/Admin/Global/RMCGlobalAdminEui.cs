...
public sealed class RMCGlobalAdminEui : BaseEui
{
    ...
    public override void HandleState(EuiStateBase state)
    {
        ...
        _window.MarinesLabel.Text = Loc.GetString("ssmc-admin-count-marines", ("cmarines", s.Marines));
        ...
        {
            _window.XenoTiers.AddChild(new Label { Text = Loc.GetString("ssmc-admin-count-xenos-by-tier", ("tier", tier), ("amount", amount)) });
        ...
        _window.XenosLabel.Text = Loc.GetString("ssmc-admin-count-xenos", ("cxenos", s.Xenos.Count));

        foreach (var (guid, actor, round) in s.TacticalMapHistory)
        {
            var lines = new Button { Text = Loc.GetString("ssmc-admin-tacmap-history-selected", ("roundid", round), ("author", actor)) };
        ...
        {
            _window.TacticalMapLabel.Text = Loc.GetString("ssmc-admin-tacmap-history-selected-none");
        }
        else
        {
            _window.TacticalMapLabel.Text = Loc.GetString("ssmc-admin-tacmap-history-selected", ("roundid", s.TacticalMapLines.RoundId), ("author", s.TacticalMapLines.Actor));
