...
public sealed class IdModificationConsoleBui : BoundUserInterface, IRefreshableBui
{
    ...
    public void Refresh()
    {
        ...
        if (console.Authenticated)
            _window.SignInButton.Text = Loc.GetString("ssmc-id-console-log-out");
        else if (privileged != null)
            _window.SignInButton.Text = Loc.GetString("ssmc-id-console-eject-card");
        else
            _window.SignInButton.Text = Loc.GetString("ssmc-id-console-sign-in");

        if (target != null)
        {
            var entityName = metaData?.EntityName ?? Loc.GetString("ssmc-id-console-unknown-name");
            var fullName = targetCardComponent?.FullName ?? Loc.GetString("ssmc-id-console-unknown-name");

            _window.SignInTargetButton.Text = Loc.GetString("ssmc-id-console-eject-card-by-name", ("entityname", entityName));
            _window.SignInTargetAccount.Text = Loc.GetString("ssmc-id-console-account-number", ("fullname", fullName)); //todo RMC14 Account Numbers
            _window.SignInTargetName.Text = fullName;
        }
        else
        {
            _window.SignInTargetButton.Text = Loc.GetString("ssmc-id-console-insert-to-modify");
            _window.SignInTargetAccount.Text = Loc.GetString("ssmc-id-console-no-card-inserted");
    ...
    protected override void Open()
    {
        ...
        tab1.TabButton.Text = Loc.GetString("ssmc-id-console-access");
        ...
        tab2.TabButton.Text = Loc.GetString("ssmc-id-console-jobs");
        ...
        {
            _window.TerminateConfirm.Text = Loc.GetString("ssmc-id-console-jobs-id-terminated");
        ...
        tab4.TabButton.Text = Loc.GetString("ssmc-id-console-squads");
    ...
    private void RefreshIFFButton(IdModificationConsoleComponent console)
    {
        ...
        if (console.HasIFF)
        {
            ...
            _window.IFF.Text = Loc.GetString("ssmc-id-console-revoke-iff");
            return;
        }

        _window.IFF.ModulateSelfOverride = Color.Green;
        _window.IFF.Text = Loc.GetString("ssmc-id-console-grant-iff");
    }
