...
public sealed class StaffHelpUIController : UIController, IOnSystemChanged<BwoinkSystem>
{
    ...
    private void UpdateClaimIndicator(NetUserId destination)
    {
        ...
        _mentorWindow.ClaimButton.Text = Loc.GetString("ssmc-mentor-claim");
        ...
        {
            _mentorWindow.ClaimButton.Text = Loc.GetString("ssmc-mentor-unclaim");
        }
        ...
        _mentorWindow.ClaimIndicator.Text = $"{Loc.GetString("ssmc-mentor-claimed-by")} {string.Join(", ", claims)}";
    }
}
