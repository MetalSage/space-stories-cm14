...
internal sealed class RMCCrtEffectRenderer
{
    ...
    public void Draw(DrawingHandleScreen handle, float width, float height, float uiScale,
        RMCCrtEffects effects, float scanlineSpacing, float scanlineThickness, float rgbWidth,
        float stripeWidth, float scanlineOpacity, float rgbOpacity, Color stripeColor)
    {
        UpdateGeometry(width, height, uiScale, effects, scanlineSpacing,
            scanlineThickness, rgbWidth, stripeWidth);

        if (_horizontalScanlines.Length > 0)
            handle.DrawPrimitives(DrawPrimitiveTopology.TriangleList,
                _horizontalScanlines, Color.Black.WithAlpha(
                Math.Clamp(scanlineOpacity, 0, 1)));

        if (_redSubpixels.Length > 0)
            handle.DrawPrimitives(DrawPrimitiveTopology.TriangleList,
                _redSubpixels, Color.Red.WithAlpha(Math.Clamp(rgbOpacity, 0, 1)));
        if (_greenSubpixels.Length > 0)
            handle.DrawPrimitives(DrawPrimitiveTopology.TriangleList,
                _greenSubpixels, Color.Lime.WithAlpha(Math.Clamp(rgbOpacity * 0.35f, 0, 1)));
        if (_blueSubpixels.Length > 0)
            handle.DrawPrimitives(DrawPrimitiveTopology.TriangleList,
                _blueSubpixels, Color.Blue.WithAlpha(Math.Clamp(rgbOpacity, 0, 1)));

        if (_diagonalStripes.Length > 0)
            handle.DrawPrimitives(DrawPrimitiveTopology.TriangleList,
                _diagonalStripes, stripeColor);
    }

    internal void UpdateGeometry(float width, float height, float uiScale, RMCCrtEffects effects,
        float scanlineSpacing, float scanlineThickness, float rgbWidth, float stripeWidth)
    {
    ...
    private static Vector2[] CreateHorizontalLines(float width, float height, float uiScale,
        float spacing, float thickness)
    {
        ...
        for (var y = 1f; y < height; y += scaledSpacing)
            AddRectangle(vertices, 0, y, width,
                Math.Min(y + scaledThickness, height));
