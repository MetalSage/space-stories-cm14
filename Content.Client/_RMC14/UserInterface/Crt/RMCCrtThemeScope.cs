...
public sealed class RMCCrtThemeScope : PanelContainer
{
    ...
    public RMCCrtPalettePreset Palette { get => _palette; set { _palette = value; RefreshTheme(); } }
    public RMCCrtEffects Effects { get => _effects; set => _effects = value; }
    public float BorderThickness { get => _borderThickness; set { _borderThickness = value; UpdatePanel(); } }
    public float BackgroundOpacity { get => _backgroundOpacity; set { _backgroundOpacity = value; UpdatePanel(); } }
    ...
    protected override void PostRenderChildren(ref ControlRenderArguments args)
    {
        ...
        // Children (labels, buttons, tab panels, etc.) render after a control's own Draw() and
        // are normally opaque, so painting the scanline overlay in Draw() left it completely
        // hidden underneath them. Drawing it here instead -- after children -- puts it on top.
        // By this point the handle's transform has already been reset to this control's parent
        // context (see UserInterfaceManager.RenderControl), so it has to be re-applied manually
        // using this control's own absolute position before drawing.
        var handle = args.Handle.DrawingHandleScreen;
        var oldXform = handle.GetTransform();
        handle.SetTransform(oldXform * Matrix3Helpers.CreateTranslation((Vector2)GlobalPixelPosition));

        _effectsRenderer.Draw(handle, PixelWidth, PixelHeight, UIScale, Effects,
            ScanlineSpacing, ScanlineThickness, RgbWidth, StripeWidth, ScanlineOpacity,
            RgbOpacity, ResolvedPalette.Warning.WithAlpha(0.3f));
    ...
    private void RefreshTheme()
    {
        ResolvedPalette = Palette == RMCCrtPalettePreset.Custom
            ? new RMCCrtPalette(CustomForeground, CustomBackground, CustomBorder, CustomFill,
                CustomFillForeground, CustomGood, CustomWarning, CustomDanger, CustomMuted,
                CustomDisabledBackground, CustomDisabledForeground)
