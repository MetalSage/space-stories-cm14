...
public sealed class CMSurgeryBui : BoundUserInterface
{
    ...
    private void Update(CMSurgeryBuiState state)
    {
        if (_window == null)
        {
            ...
            _window.Title = Loc.GetString("ssmc-medical-surgery-window-title");
    ...
    private void OnSurgeryPressed(Entity<CMSurgeryComponent> surgery, NetEntity netPart, EntProtoId surgeryId)
    {
        ...
        if (surgery.Comp.Requirement is { } requirementId && _system.GetSingleton(requirementId) is { } requirement)
        {
            ...
            msg.AddMarkupOrThrow(Loc.GetString("ssmc-medical-surgery-requires-operation", ("surgery", surgeryName)));
            label.Set(msg, null);
                    ...
                    !_system.CanPerformStep(player, Owner, part.PartType, stepButton.Step, false, out var popup, out var reason, out _))
                {
                    ...
                    switch (reason)
                    {
                        case StepInvalidReason.MissingSkills:
                            stepName.AddMarkupOrThrow(Loc.GetString("ssmc-medical-surgery-skill-issue"));
                            break;
                        case StepInvalidReason.NeedsOperatingTable:
                            stepName.AddMarkupOrThrow(Loc.GetString("ssmc-medical-surgery-need-operating-table"));
                            break;
                        case StepInvalidReason.Armor:
                            stepName.AddMarkupOrThrow(Loc.GetString("ssmc-medical-surgery-remove-armor"));
                            break;
                        case StepInvalidReason.MissingTool:
                            stepName.AddMarkupOrThrow(Loc.GetString("ssmc-medical-surgery-missing-tool"));
                            break;
                    }
                }
            }
    ...
    private void UpdateDisabledPanel()
    {
        ...
        text.AddMarkupOrThrow(Loc.GetString("ssmc-medical-surgery-need-to-be-lying-down"));
    ...
    private void View(ViewType type)
    {
        ...
        {
            _window.Title = Loc.GetString("ssmc-medical-surgery-window-title-part-meta", ("part" ,partMeta.EntityName), ("meta", surgeryMeta.EntityName));
        }
        else if (partMeta != null)
        {
            _window.Title = Loc.GetString("ssmc-medical-surgery-window-title-part", ("part" ,partMeta.EntityName));
        }
        else
        {
            _window.Title = Loc.GetString("ssmc-medical-surgery-window-title");
