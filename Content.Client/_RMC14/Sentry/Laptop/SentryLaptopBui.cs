...
public sealed class SentryLaptopBui : BoundUserInterface
{
    ...
    private void SetupGlobalIFFControls()
    {
        ...
        {
            Text = Loc.GetString("ssmc-sentry-select-all"),
            HorizontalExpand = true,
        ...
        {
            Text = Loc.GetString("ssmc-sentry-deselect-all"),
            HorizontalExpand = true,
    ...
    private void UpdateHeader(SentryLaptopBuiState state)
    {
            ...
            _entities.TryGetComponent<SentryLaptopComponent>(laptopEntity.Value, out var laptop))
        {
            ...
            _window!.LaptopHeader.SetMarkupPermissive(Loc.GetString("ssmc-sentry-network", ("count", sentryCount), ("max", maxSentries)));
        }
    }
    ...
    private void UpdateStatusLabel(SentryLaptopBuiState state)
    {
        ...
        if (state.Sentries.Count == 0)
        {
            _window!.StatusLabel.Text = Loc.GetString("ssmc-sentry-status-no-linked");
            _window.StatusLabel.FontColorOverride = Color.FromHex("#A42625");
            return;
        }

        var onlineCount = state.Sentries.Count(s => s.Mode == SentryMode.On);
        _window!.StatusLabel.Text = Loc.GetString("ssmc-sentry-status-online", ("online", onlineCount), ("allcount", state.Sentries.Count));
    ...
    private void PopulateSentryBasicInfo(SentryCard card, SentryInfo info)
    {
        ...
        {
            SentryMode.On => Loc.GetString("ssmc-sentry-mode-on"),
            SentryMode.Off => Loc.GetString("ssmc-sentry-mode-off"),
            _ => Loc.GetString("ssmc-sentry-mode-off2")
        });
    ...
    private void PopulateSentryStatusInfo(SentryCard card, SentryInfo info)
    {
        ...
        {
            SentryMode.On => Loc.GetString("ssmc-sentry-mode2-on"),
            SentryMode.Off => Loc.GetString("ssmc-sentry-mode2-off"),
            _ => Loc.GetString("ssmc-sentry-mode2-off2")
        });
        ...
        {
            card.TargetLabel.SetMarkupPermissive(Loc.GetString("ssmc-sentry-target-none"));
        }
    }
    ...
    private void ViewSentryCamera(NetEntity sentryNetEntity)
    {
        ...
        var sentryName = Loc.GetString("ssmc-sentry-name-unknown");
        ...
        _window.CameraTitle.SetMarkupPermissive(Loc.GetString("ssmc-sentry-info", ("name", sentryName), ("radius", visionRadius), ("fov", fovText)));
