...
public sealed class RequisitionsBui(EntityUid owner, Enum uiKey) : BoundUserInterface(owner, uiKey)
{
    ...
    private void UpdateState(RequisitionsBuiState uiState)
    {
        ...
        var platformLabel = Loc.GetString("ssmc-req-no-platform");
        var platformButtonLabel = Loc.GetString("ssmc-req-no-platform");
        var platformButtonDisabled = false;
        bool? raise = null;
        switch (uiState.PlatformLowered)
        {
            case Lowered or Raised when uiState.Busy:
                platformLabel = Loc.GetString("ssmc-req-platform-pos", ("position", uiState.PlatformLowered));
                platformButtonLabel = Loc.GetString("ssmc-req-asrs-is-busy");
                platformButtonDisabled = true;
                break;
            case Lowered:
                platformButtonLabel = Loc.GetString("ssmc-req-raise-platform");
                platformLabel = Loc.GetString("ssmc-req-platform-pos-low");
                raise = true;
                break;
            case Raised:
                platformButtonLabel = Loc.GetString("ssmc-req-lower-platform");
                platformLabel = Loc.GetString("ssmc-req-platform-pos-raise");
                raise = false;
                break;
            case Lowering:
                platformButtonLabel = Loc.GetString("ssmc-req-asrs-please-wait");
                platformLabel = Loc.GetString("ssmc-req-lower-platfotrm-wait");
                platformButtonDisabled = true;
                break;
            case Raising:
                platformButtonLabel = Loc.GetString("ssmc-req-asrs-please-wait");
                platformLabel = Loc.GetString("ssmc-req-raise-platform-wait");
                platformButtonDisabled = true;
                break;
            case null:
                platformButtonDisabled = true;
        ...
        budget.AddMarkupOrThrow(Loc.GetString("ssmc-req-supply-budget", ("balance", uiState.Balance)));
        ...
        categoryHeader.AddMarkupOrThrow(Loc.GetString("ssmc-req-select-category"));
    ...
    private void ChangeOrderCategory(int categoryIndex)
    {
        ...
        requestMsg.AddMarkupOrThrow(Loc.GetString("ssmc-req-request-from-category", ("category", category.Name)));
    ...
    private void UpdateItemSearch(string? filter = null)
    {
        ...
        for (var catIndex = 0; catIndex < computer.Categories.Count; catIndex++)
        {
            ...
            var categoryHeader = new FormattedMessage();
            categoryHeader.AddMarkupOrThrow(Loc.GetString("ssmc-req-request-from-category", ("category", category.Name)));
